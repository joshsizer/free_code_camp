...
def sym(*args):
    ...
    of two or more sets. If only one set
    is input, the same set is returned
    unmodified. If no set is input, null
    is returned.
    """
    # Return None if the length of args
    # is 0
    if not args:
        return None
    ...
    # Add each argument (set) onto the
    # stack
    for arg in args:
        stack.append(arg)
    ...
    # Compare sets until our stack is
    # empty
    while stack:
        # Get the set to compare to the
        # current
